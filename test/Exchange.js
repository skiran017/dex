...
describe('Exchange', () => {
  let deployer, feeAccount, accounts, exchange, token1, token2, user1, user2;
  ...
  beforeEach(async () => {
    ...
    token1 = await Token.deploy('Spektr Token', 'SPKTR', '999999999');
  ...
  describe('Making Orders', () => {
    ...
    describe('Failure', () => {
      it('fails for insufficient balance', async () => {
        //Attempt to place order without tokens
        await expect(
          exchange
            .connect(user1)
            .makeOrder(token2.address, tokens(1), token1.address, tokens(1))
